...
import { getContactsAsync } from './contacts.actions';
...
function* searchContactsWorker(socket: Socket, action: ReturnType<typeof searchContactsAsync>) {
  if (socket) {
...
function* addContactWorker(action: ReturnType<typeof addContactAsync>) {
...
function* deleteContactWorker(action: ReturnType<typeof deleteContactAsync>) {
  try {
    yield put(startLoading('loading'));
    const response: AxiosResponse<IContact[]> = yield call(
...
export function* contactsSagaWatcher(): Generator<any, void, any> {
  ...
  yield takeEvery(addContactAsync.type, addContactWorker);
  yield takeEvery(deleteContactAsync.type, deleteContactWorker);
  yield takeEvery(getContactsAsync.type, getContactsWorker);
  ...
  while (true) {
    ...
    const action: ReturnType<typeof searchContactsAsync> = yield take(searchContactsAsync.type)
